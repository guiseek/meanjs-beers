...
exports.list = function(req, res) { 
	Beer.find().sort('-created').populate('user', 'displayName').populate('brewery', 'name').exec(function(err, beers) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.beerByID = function(req, res, next, id) { 
	Beer.findById(id).populate('user', 'displayName').populate('brewery', 'name').exec(function(err, beer) {
